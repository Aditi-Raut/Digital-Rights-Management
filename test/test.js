...
contract('DStorage', ([deployer, uploader, subscriber]) => {
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await dstorage.uploadFile(fileHash, fileType, fileName, fileDescription, { from: uploader })
    ...
    it('upload file', async () => {
      ...
      await dstorage.uploadFile('', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await dstorage.uploadFile(fileHash, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await dstorage.uploadFile(fileHash, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await dstorage.uploadFile(fileHash, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })
    ...
    it('lists file', async () => {
      ...
      assert.equal(file.views, 0, 'uploader is correct')
    ...
    it('donates funds', async () => {
     ...
     it('adds views', async () => {
      ...
      //Success: Buyer makes purchase
      result = await dstorage.viewed(fileCount,{ from: subscriber, value: web3.utils.toWei('1','Ether') });
      ...
      assert.equal(event.uploader, uploader, 'Uploader is correct')
      assert.equal(event.owner, uploader, 'Uploader is correct')
      assert.equal(event.views, 1, 'views is correct')
      assert.equal(event.viewPrice, 10, 'viewPrice is correct')
      //Check if seller received funds
      let newOwnerBalance 
      newOwnerBalance= await web3.eth.getBalance(uploader);
      newOwnerBalance = new web3.utils.BN(newOwnerBalance);
      ...
      //Failure:Tries to buy a product that does not exist i.e product must have valid id
      await dstorage.viewed(99,{ from: subscriber, value: web3.utils.toWei('1','Ether')  }).should.be.rejected;
      //Failure: Tries to buy with not enough ether
      await dstorage.viewed(fileCount,{ from: subscriber, value: web3.utils.toWei('0','Ether')  }).should.be.rejected;
      //Failure Buyer can't be the seller
      await await dstorage.viewed(fileCount,{ from: uploader, value: web3.utils.toWei('1','Ether')  }).should.be.rejected;
